...
# PTTnews網頁中抓取有關胎心率的相關文章
def PTTnews():
    total = []
    for i in range(1, 4):
        url = requests.get("https://pttnews.cc/s/%E8%83%8E%E5%BF%83%E7%8E%87/" + str(i))   # 用requests的get方法把網頁抓下來
        html = BeautifulSoup(url.text, "html.parser")   # html.parser作為解析器
        #print(html)
        st = html.find_all("a",target="_blank")   # .find_all 找所有符合的
        for i in st:
            title = i.find(class_="article-item-title")
            url = "https://pttnews.cc/" + i.get("href")
            title1 = title.text
            total.append(title1+'\n'+url)
            #print(total)
    return total
...
def process_follow_event(event):
    ...
    # 讀取並轉換，若要將圖文選單加在第一頁關注底下，須再加上164及165兩條程式(可至258、259copy)
    # 在生成id檔案底下修改：rich_menu_array=['關注']
...
def process_postback_event(event):
    #======
#     if event.postback.data == "02_文章推薦":
#         content = PTTnews()
#         for c in content:
#             #print(c)
#             line_bot_api.reply_message(
...
#     print("query_string_dict@@@",query_string_dict)
    ...
    elif 'paper' in query_string_dict:
        content = PTTnews()   
        for c in content:
            line_bot_api.reply_message(
                event.reply_token,
...
#      elif query_string_dict == "02_文章推薦":
#         
#         for c in content:
#             #print(c)
#             line_bot_api.reply_message(
...
with open(message_from_user, 'wb') as fd:
